...
le pagine di registration chiedono per entrambi gli utenti i dati base mail pass nome cognome e a seconda 
del tipo FC o matricola. La pagina di default che viene mostrata se dal login si clicca su register è quella del 
citizen, se ci si vuole registrare come authoriy si può cliccare sul bottone reegistrati come autorita. Dalla pagina 
citizen si può andare nella pagina aut cliccando sul bottone aut, viceversa dalla pagina authority si potrà
andare sulla pagina registrazione utente cliccando sul bottone register citizen. 
\\
\\
\\
\\
...
\begin{itemize}
    \item \textbf{Citizen}: la home page mostra al cittadino una barra in alto con alcuni dati di interesse, tra coupling
    nome e cognome codice fiscale e il numero totale di violazioni riportate. Sotto l'applicazione mostra la fotocamera che
    è sempre attiva e pronta per scattare se viene inquadrata una targa cliccando il tasto report altrimenti non sarà 
    possibile scattare la foto. Il bottone di report al di sotto, scatterà la foto e reindirizzerà il citizen nella
    pagina di conferma e inserimento dati. Gli ultimi due bottoni in basso permettono di accedere alle statistiche o
    alle proprie impostazioni dell'account.
    \item \textbf{Authority}: Per gli autority la home invece mostra comunque una barra con informazioni quali nome cognome
    e matricola ma a differenza del Citizen mostra il numero di violazioni controllate e di quelle confermate., al di sotto
    il bottone retrieve violations permette di accedere alla pagina di valutazione violazioni, mentre i due bottoni di sotto
    servono per accedere alle statistiche che nel caso dell'authority possono riguardare statistics di violazioni e statistics
    di vehicles. è inoltre presente il bottone per accedere alle proprie impostazioni.
\end{itemize}
...
\textbf{Flow of events} & 1. \textit{User} opens the app \\
                        & 2. \textit{User} clicks the login button  \\
                        & 3. \textit{System} shows the form \\
                        & 4. \textit{User} inputs his credentials  \\
                        & 5. \textit{System} checks the validity of the data inserted \\
\hline
